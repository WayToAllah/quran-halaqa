...
منقول 1:1 من الكود الحي) مع **118 اختبار Vitest**، بما فيها إصلاح مضمّن ومختبر لـ
`scoreName(0)`.
...
- ✅ **شاشة الإحصاءات** (`features/stats/`) — فلتر شهري، بطاقات ملخّص، رسم أسبوعي،
  توزيع التقييمات، لوحتا شرف (آيات/حضور)، جدول طلاب قابل للبحث والترتيب — منطق كامل
  منقول لطبقة `domain/statsScreen.ts` الصافية (20 اختبار وحدة) + 9 اختبار مكوّن
- ⏳ **تسجيل** — لسه شاشة "قريباً" (placeholder) في `app.tsx` — آخر شاشة متبقية
...
npm test                    # اختبارات domain الصافية — 118 اختبار
npm run test:components     # اختبارات المكوّنات (Testing Library + happy-dom) — 34 اختبار
...
src/
  ...
  domain/              ← منطق صافٍ 100%، بلا DOM ولا Firebase (118 اختبار)
    statsScreen.ts         ← حسابات شاشة الإحصاءات (ملخّص، رسم أسبوعي، توزيع، لوحات شرف)
  ...
  features/
    ...
    stats/               StatsScreen.tsx (+ 9 اختبار)
...
- **"🌟 نجوم الحضور"** (بطاقة مشاركة صورة عبر html2canvas في التطبيق الحي) لم تُنقل
  بعد — ميزة منفصلة نسبياً (توليد صورة)، مؤجلة عمداً لتركيز الجهد على الشاشات
  الأساسية أولاً.

## الخطوات القادمة

- شاشة **تسجيل** (الأعقد والأخيرة — student picker، صفوف لوح/ماضي متعددة، التجويد)
